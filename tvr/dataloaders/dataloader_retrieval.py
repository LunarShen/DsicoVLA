...
#         while len(input_ids) < self.max_words:
    ...
    def _get_f_text(self, f_captions):
        input_ids_list = []
        input_mask_list = []
        for caption_text in f_captions:
...
#         while len(input_ids) < self.max_words:
        ...
        input_mask = np.stack(input_mask_list, axis=0)
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'all':
            if self.subset == 'train':
                video_id, caption, f_captions = self.sentences_dict[idx]
                text_ids, text_mask, s, e = self._get_text(caption)
                f_text_ids, f_text_mask = self._get_f_text(f_captions)
                video, video_mask = self._get_rawvideo_dec(video_id, s, e)
                # video, video_mask = self._get_rawvideo(video_id, s, e)
                return text_ids, text_mask, f_text_ids, f_text_mask, video, video_mask, idx, hash(video_id.replace("video", ""))
            else:
