...
from .module_GlobalCLS import ResidualAttentionBlock_GlobalCLS
import torch
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, lora_dim: int, global_layers: int, attn_mask=None):
        super(Transformer, self).__init__()
        self.width = width
        self.layers = layers
        
        # self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, lora_dim, attn_mask) for _ in range(layers)])
        resblocks_list = [ResidualAttentionBlock(width, heads, lora_dim, attn_mask) for _ in range(layers-global_layers)] + \
                            [ResidualAttentionBlock_GlobalCLS(width, heads, lora_dim, attn_mask) for _ in range(global_layers)]
...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, lora_dim: int, global_layers: int):
        ...
        self.transformer = Transformer(width, layers, heads, lora_dim, global_layers)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # self.initialize_parameters() ### lora init
