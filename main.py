...
def get_args(description='DiscoVLA: Discrepancy Reduction in Vision, Language, and Alignment for Parameter-Efficient Video-Text Retrieval'):
    ...
    parser.add_argument('--pretrained_path', type=str, default="/home/shenleqi/disk/data/pretrained", help='pretrained model path')
    parser.add_argument('--PseudoImageCaptions_path', type=str, default="/home/shenleqi/disk/data/pretrained")

    parser.add_argument('--seed', type=int, default=42, help='random seed')
    parser.add_argument('--workers', default=4, type=int, help='number of data loading workers (default: 4)')
    parser.add_argument('--lr', type=float, default=1e-4, help='initial learning rate')
    parser.add_argument('--clip_lr', type=float, default=6e-4, help='learning rate for bert branch.')
    ...
    parser.add_argument('--lora_adapter_dim', type=int, default=8)

    parser.add_argument('--H_V', type=int, default=4)
    parser.add_argument('--H_L', type=int, default=2)
...
def prep_optimizer(args, model, num_train_optimization_steps, local_rank):
    ...
    logger.info(f"Prompt Parameters to be updated: {sorted(enabled_prompt)}")
...
def save_model(epoch, args, model, type_name=""):
    ...
    logger.info("Not Save %s", output_model_file)
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer,
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        text_ids, text_mask, f_text_ids, f_text_mask, video, video_mask, inds, idx = batch
        loss, loss_videoLframevideoV, loss_frameLframeV, distillation_logits = model(text_ids, text_mask, f_text_ids, f_text_mask, video, video_mask, idx, global_step)
        ...
        if n_gpu > 1:
            ...
            loss_videoLframevideoV = loss_videoLframevideoV.mean()
            loss_frameLframeV = loss_frameLframeV.mean()
            distillation_logits = distillation_logits.mean()
...
#         optimizer.zero_grad()
        ...
        reduced_loss_videoLframevideoV = reduce_loss(loss_videoLframevideoV, args)
        reduced_loss_frameLframeV = reduce_loss(loss_frameLframeV, args)
        reduced_distillation_logits = reduce_loss(distillation_logits, args)
        meters.update(time=batch_time, data=data_time, loss=float(reduced_l),
                                                     videoLframevideoV=float(reduced_loss_videoLframevideoV),
                                                     frameLframeV=float(reduced_loss_frameLframeV),
                                                     distillation_logits=float(reduced_distillation_logits))
...
def eval_epoch(args, model, test_dataloader, device):
    ...
    with torch.no_grad():
        ...
        # args.batch_size_val // args.world_size
    ...
    max_R1=[]
    # for list_idx, sim_matrix in enumerate(sim_matrix_list):
    list_idx = 0
...
def main():
    ...
    logger.info("{}/{} M Params".format(trainable_params/1e6, total_params/1e6))
    ...
    if args.do_train:
        ...
        logger.info("Prompt: {} Params".format(trainable_params))
